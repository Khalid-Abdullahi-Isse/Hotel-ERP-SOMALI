...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  return { title: entry.entryNumber };
}
...
export default async function JournalEntryDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const baseCurrency = entry.lines[0]?.currency ?? "USD";
  return (
    <div className="space-y-6">
      <PageHeader
        title={entry.entryNumber}
        description="Journal entry detail and permanent posting history."
        actions={
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
          ...
          <Card>
            <CardHeader className="border-b">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <Badge
                  variant={entry.status === "POSTED" ? "secondary" : entry.status === "REVERSED" ? "destructive" : "outline"}
                >
                  {entry.status}
                </Badge>
              </div>
            ...
            <CardContent className="grid gap-4 pt-2 sm:grid-cols-3">
              <Meta icon={CalendarDays} label="Business date" value={entry.businessDate.slice(0, 10)} />
              <Meta icon={FileText} label="Reference" value={entry.reference || "No reference"} />
              <Meta icon={UserRound} label="Created by" value={entry.createdBy.fullName} />
            </CardContent>
          </Card>
                ...
                {entry.lines.map((line) => (
                  <TableRow key={line.id}>
                    ...
                    <TableCell className="text-right font-mono tabular-nums">
                      {Number(line.debit) !== 0 ? accountingMoney(line.debit, line.currency) : "—"}
                    </TableCell>
                    <TableCell className="text-right font-mono tabular-nums">
                      {Number(line.credit) !== 0 ? accountingMoney(line.credit, line.currency) : "—"}
                    </TableCell>
                    <TableCell className="text-right font-mono tabular-nums">{line.currency}</TableCell>
                    <TableCell className="text-right font-mono tabular-nums">
              ...
              <TableFooter>
                <TableRow>
                  ...
                  <TableCell colSpan={2} className="text-right">
                    <span
                      className={
                        Number(entry.difference) === 0 ? "text-green-600" : "text-destructive"
                      }
                    >
                      {Number(entry.difference) === 0 ? "Balanced" : `Diff ${entry.difference}`}
                    </span>
                  </TableCell>
                </TableRow>
              </TableFooter>
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>Ledger action</CardTitle>
            </CardHeader>
            <CardContent>
              <JournalEntryActions
                entryId={entry.id}
                status={entry.status}
                canPost={Boolean(user && can(user, PERMISSIONS.journalsPost))}
                canReverse={Boolean(user && can(user, PERMISSIONS.journalsReverse))}
              />
              <p className="mt-3 text-xs leading-5 text-muted-foreground">
                Posted entries are immutable. Corrections are made with a linked reversal.
              </p>
            </CardContent>
          </Card>
              ...
              <CardContent className="text-sm">
                ...
                {entry.reversalEntry ? (
                  <Button className="mt-3 px-0" variant="link" asChild>
                    <Link href={`/accounting/journal-entries/${entry.reversalEntry.id}`}>
                      View {entry.reversalEntry.entryNumber}
                    </Link>
                  </Button>
                ) : null}
              </CardContent>
