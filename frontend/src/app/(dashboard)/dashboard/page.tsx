...
export default async function DashboardPage() {
  ...
  const reservations = await getReservations({ page: 1, pageSize: 5, arrivalFrom: summary.businessDate });
  ...
  const metrics: DashboardMetric[] = [
    ...
    { id: "revenue", label: "Net revenue", value: formatCurrency(Number(summary.financial.net), summary.currencyCode), supportingText: "for this business date", icon: "revenue" },
  ];
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Dashboard"
        description={`Saved hotel records for ${summary.businessDate} (${summary.timezone}).`}
        actions={<Button asChild><Link href="/reservations/new">New reservation</Link></Button>}
      />
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.5fr)_minmax(280px,0.7fr)]">
        <RecentReservations reservations={reservations.data} title="Upcoming reservations" description="Saved bookings ordered by arrival date" />
        <RoomStatusOverview statuses={roomStatuses} />
      </div>
      <div className="grid gap-4 md:grid-cols-2">
        {[
          { title: "Housekeeping", count: housekeeping, detail: "open tasks", href: "/housekeeping", icon: ClipboardCheck },
          { title: "Maintenance", count: maintenance, detail: "open requests", href: "/maintenance", icon: Wrench },
