      ...
      <CardContent>
        <div className="relative h-[190px] w-full" role="img" aria-label={`Occupancy ranged from ${Math.min(...points.map((point) => point.value))}% to ${Math.max(...points.map((point) => point.value))}% over seven days`}>
          ...
          <svg viewBox="0 0 600 170" preserveAspectRatio="none" className="absolute inset-x-8 top-0 h-[170px] w-[calc(100%-2rem)] overflow-visible" aria-hidden="true">
            <defs><linearGradient id="occupancy-fill" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#0f766e" stopOpacity="0.22" /><stop offset="100%" stopColor="#0f766e" stopOpacity="0" /></linearGradient></defs>
            <path d={area} fill="url(#occupancy-fill)" /><path d={line} fill="none" stroke="#0f766e" strokeWidth="3" vectorEffect="non-scaling-stroke" strokeLinecap="round" strokeLinejoin="round" />
            {coordinates.map((point, index) => <circle key={points[index].label} cx={point.x} cy={point.y} r="4" fill="white" stroke="#0f766e" strokeWidth="2" vectorEffect="non-scaling-stroke" />)}
          </svg>
